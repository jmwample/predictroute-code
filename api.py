...
def run_trace_from_all_ripe(ip_in_prefix, sources, in_parts=False):
    ...
    if in_parts:
        ...
        for i in range(0, 4):
            ...
            print "Running traces to %s from %d probes" % (
                prefix, len(sources_chunk))
...
def parse_traceroute_v2(iplinks, granularity="asn"):
    ...
    while exists_one_hop_cycle(links_new, granularity):
        links_new, removed_cycle_count = short_circuit_one_hop_cycles(
            links_new, granularity)
        if not links_new:
            break
    linkssane = links_new
...
def exists_one_hop_cycle(links, granularity):
    ...
    for rep_hop in repeated_hops:
        occurences_of_rep_hop = [
            i for i, x in enumerate(loopdetect) if x == rep_hop]
...
def add_node_to_nodelinks(links, nodes, node, position="front"):
    if position == "front":
        ...
        new_link = {'src': node, 'dst': dst_node,
                    'type': 'i'}  # always assume indirect
...
def remove_ixps(nodelinks):
    ...
    nodes = [node for node in nodelinks['_nodes']
             if node not in identified_ixps]
...
def does_target_match(trace_target, node_gran, dag_target):
    ...
    if node_gran == 'asn':
        ...
        if ".".join(trace_target.split('.')[:-1]) == ".".join(dag_target.split(".")[:-1]):
            return True
    print "Trace target: %s and DAG target: %s" % (
        trace_target_bgp_pfx, dag_target)
    return False
...
def compute_dest_based_graphs(dst_ip,
    ...
    if dst_ip and not global_dst_iden_attr:
        ...
        global_dst_pfx, global_dst_bgp_pfx, global_dst_asn = get_pref_asn(
            dst_ip, ts)
    ...
    if search_for_msms:
        ...
        msms_all.extend(get_msms(global_dst_asn, ts,
                        mmt_window_hours=mmt_search_window_hrs))
    ...
    for msm in msms_all:
        ...
        for d in data:
            ...
            trace_hour_count = datetime.datetime.fromtimestamp(
                trace_endtime).hour
            ...
            dst_pfx, dst_bgp_pfx, dst_asn = get_pref_asn(
                measurement_target, ts)
            if not dst_bgp_pfx or not dst_asn:
                continue
            ...
            dst_iden_attr = get_iden_attr(
                dst_asn, dst_bgp_pfx, dst_pfx, node_gran)
            ...
            root_node = create_new_node(
                G, dst_asn, dst_bgp_pfx, dst_pfx, node_gran)
            ...
            src_node_dict = {'asn': str(
                src_asn), 'bgp_pfx': src_bgp_pfx, 'pfx': None}
            ...
            if node_gran == 'bgp_pfx' and src_bgp_pfx and \
               firstlink['src']['bgp_pfx'] != src_bgp_pfx:
                nodelinks, nodes = add_node_to_nodelinks(
                    nodelinks, nodes, src_node_dict)
            ...
            if str(nodelinks[-1]['dst'][node_gran]) != str(dst_iden_attr):
                if node_gran == 'asn' and are_siblings(str(nodelinks[-1]['dst']['asn']),
                    ...
                    ases_where_traces_go_to_die.add(
            ...
            for link in nodelinks:
                ...
                prev_edge = "%s-%s" % (link['src']
                                       [node_gran], link['dst'][node_gran])
...
def get_path_prob(gr, path):
    ...
    for x, y in zip(path, path[1:]):
        ...
        # Laplacian smoothing
        prob = float(traversed_on_edge + 1) / \
            (incident_on_src + source.out_degree())
        assert prob > 0
        assert prob <= 1
    ...
    while True:
        if not prev_path:
            vlist, elist = shortest_path(
                gr, src_node, dst_node, weights=gr.ep.weight)
            if vlist:
            ...
            for i in range(1, len(prev_path)):
                ...
                if spur_vlist:
                    try:
                        ...
                        print "MAD MAD YEN"
                        pdb.set_trace()
                    complete_path = root[:-1] + \
                        [gr.vertex(int(x)) for x in spur_vlist]
...
def get_ranked_paths(gr, src_nodes, dst_node):
    ...
    if dst_node.in_degree() == 0:
        assert False, "destination node is disconnected, WTF " + \
            gr.vp.bgp_pfx[dst_node]
...
def can_split_node(gr, node):
    ...
    for origin_set_key_1 in origin_dict:
        for origin_set_key_2 in origin_dict:
            if origin_set_key_1 == origin_set_key_2:
                continue
...
def is_splittable_graph(gr):
    ...
    nodes_violating_dest_based_routing = set(
        nodes_violating_dest_based_routing)
...
def split_to_tree(gr, fname, save_to_disk=True):
    ...
    for node in nodes:
        if node.out_degree() == 1:
            continue
        if node.out_degree() == 0 and gr.vp.prefix[node]:
            continue
        ...
        # It cannot be split already
        assert gr_copy.vp.duplicate[node_in_copy] == 0
...
def parse_mmt_metadata_ripe(fname, mmt_type='traceroute'):
    ...
    with open(fname) as fi:
        for row in fi:
            ...
            if mdata_row['type']['name'] != mmt_type:
                continue
...
def send_traceroute_write_job(mmt_id, start=None, end=None, beanstalkport=14711):
    if start and end:
        is_success, results = AtlasResultsRequest(
            msm_id=mmt_id, start=start, end=end).create()
    else:
...
def compute_dest_based_graphs_offline(fname,
    ...
    with bz2.BZ2File(fname) as fi:
        for line in fi:
            ...
            src_node_dict = {'asn': str(
                src_asn), 'bgp_pfx': src_bgp_pfx, 'pfx': src_pfx}
            ...
            for link in nodelinks:
                ...
                if not previous_src:
                    ...
                    key = previous_src
                    prev_hop_vertex = find_vertex(
                        G, G.vp[node_gran], previous_src)
                    assert len(prev_hop_vertex) == 1
                    prev_hop_vertex = prev_hop_vertex[0]
                    assert G.edge(prev_hop_vertex,
                                  v1), "Edge should exist here."
                else:
                    key = 'unknown'
